...
public:
    ...
    void setAppSelectedCallback(AppSelectedCb cb)
    {
        onAppSelected = std::move(cb);
    }
    ...
    void draw()
    {
        ...
        if (activeSubmenu >= 0 && activeSubmenu < itemCount && menuItems[activeSubmenu].hasSubmenu())
        {
...
private:
    ...
    static void drawMenuPanel(LGFX_Sprite &c, int mx, int my, int mw, int mh, const std::vector<MenuItem> &items,
                              int hlIndex)
    {
        ...
        for (int i = 0; i < (int)items.size(); i++)
        {
            ...
            if (items[i].hasSubmenu())
            {
                ...
                c.fillTriangle(arrowX, arrowCY - 3, arrowX, arrowCY + 3, arrowX + 4, arrowCY, arrowCol);
            }
