...
namespace UI
{
    ...
    public:
        ...
        void setItems(const std::vector<FileItem> &items) { _items = items; scrollOffset = 0; selectedIndex = -1; }
        const std::vector<FileItem> &getItems() const { return _items; }

        void setViewMode(FileListViewMode mode) { viewMode = mode; scrollOffset = 0; }
        FileListViewMode getViewMode() const { return viewMode; }

        void setOnItemSelected(ItemSelectedCb cb) { onItemSelected = std::move(cb); }
        void setOnItemActivated(ItemActivatedCb cb) { onItemActivated = std::move(cb); }
