...
public:
    ...
    void setOnKeyPress(KeyPressCb cb)
    {
        onKeyPress = std::move(cb);
    }
...
private:
    ...
    static constexpr KeyDef letterRow0[10] = {{"Q", 'q', 1}, {"W", 'w', 1}, {"E", 'e', 1}, {"R", 'r', 1},
                                              {"T", 't', 1}, {"Y", 'y', 1}, {"U", 'u', 1}, {"I", 'i', 1},
                                              {"O", 'o', 1}, {"P", 'p', 1}};

    static constexpr int kLetterRow1Count = 9;
    static constexpr KeyDef letterRow1[9] = {{"A", 'a', 1}, {"S", 's', 1}, {"D", 'd', 1}, {"F", 'f', 1}, {"G", 'g', 1},
                                             {"H", 'h', 1}, {"J", 'j', 1}, {"K", 'k', 1}, {"L", 'l', 1}};

    static constexpr int kLetterRow2Count = 9;
    static constexpr KeyDef letterRow2[9] = {{"Sh", 0, 2}, // shift (special)
                                             {"Z", 'z', 1}, {"X", 'x', 1}, {"C", 'c', 1}, {"V", 'v', 1},
                                             {"B", 'b', 1}, {"N", 'n', 1}, {"M", 'm', 1}, {"<-", '\b', 2}}; // backspace
    ...
    static constexpr KeyDef symRow0[10] = {{"1", '1', 1}, {"2", '2', 1}, {"3", '3', 1}, {"4", '4', 1}, {"5", '5', 1},
                                           {"6", '6', 1}, {"7", '7', 1}, {"8", '8', 1}, {"9", '9', 1}, {"0", '0', 1}};

    static constexpr int kSymRow1Count = 10;
    static constexpr KeyDef symRow1[10] = {{"@", '@', 1}, {"#", '#', 1}, {"$", '$', 1}, {"%", '%', 1}, {"&", '&', 1},
                                           {"-", '-', 1}, {"_", '_', 1}, {"+", '+', 1}, {"(", '(', 1}, {")", ')', 1}};

    static constexpr int kSymRow2Count = 9;
    static constexpr KeyDef symRow2[9] = {{"=", '=', 1}, {"*", '*', 1}, {"\"", '"', 1}, {"'", '\'', 1}, {":", ':', 1},
                                          {";", ';', 1}, {"!", '!', 1}, {"?", '?', 1},  {"<-", '\b', 2}}; // backspace
    ...
    void drawKey(LGFX_Sprite &c, int kx, int ky, int kw, int kh, const char *label, bool pressed) const
    {
    ...
    void drawRow(LGFX_Sprite &c, int rowY, const KeyDef *keys, int count, int baseIndex) const
    {
    ...
    int hitTestRow(int px, int py, int rowY, const KeyDef *keys, int count, int baseIndex) const
    {
    ...
    const KeyDef &keyDefAt(int flatIndex) const
    {
        if (symbolMode)
        {
            if (flatIndex < kSymRow0Count)
                return symRow0[flatIndex];
            flatIndex -= kSymRow0Count;
            if (flatIndex < kSymRow1Count)
                return symRow1[flatIndex];
            flatIndex -= kSymRow1Count;
            if (flatIndex < kSymRow2Count)
                return symRow2[flatIndex];
            flatIndex -= kSymRow2Count;
            return symRow3[flatIndex];
        }
        else
        {
            if (flatIndex < kLetterRow0Count)
                return letterRow0[flatIndex];
            flatIndex -= kLetterRow0Count;
            if (flatIndex < kLetterRow1Count)
                return letterRow1[flatIndex];
            flatIndex -= kLetterRow1Count;
            if (flatIndex < kLetterRow2Count)
                return letterRow2[flatIndex];
